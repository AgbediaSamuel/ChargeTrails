from ocr import get_receipt_text, encode_image_upload
import json
...
async def populate_db(image: image):
...
@app.post("/users/", status_code=status.HTTP_201_CREATED, response_model=UsersBase)
async def create_user(user: UsersBase):
...
@app.post("/categories/", status_code=status.HTTP_201_CREATED, response_model=CategoryBase)
...
@app.post("/products/", status_code=status.HTTP_201_CREATED, response_model=None)
...
@app.post("/locations/", status_code=status.HTTP_201_CREATED, response_model=None)
...
async def create_product_receipt(product_receipt: ProductReceiptBase):
...
if __name__ == "__main__":
    b64_image = encode_image_upload("receipt_testing.jpeg")
